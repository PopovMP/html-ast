...
const closedBy = {
    td: ["td", "th"],
    th: ["td", "th"],
    p : ["address", "article", "aside", "blockquote", "div", "dl", "fieldset", "footer", "form", "h1", "h2", "h3",
         "h4", "h5", "h6", "header", "hr", "menu", "nav", "ol", "pre", "section", "table", "ul", "p"
    ],
};

const closeIfLastChild = ["td", "th", "p"];
 ...
 * @param {ASTElement} element - The parent HTML element
 * @returns {[number, boolean]} The position after the parsed content, and if closing omission
 */
function parseElements(str, pos, element) {
    while (pos < str.length) {
        ...
        pos = parseTextNode(str, pos, element.children);
        ...
        // Check for closing by another opening tag
        if (closedBy[element.tagName]?.includes(tagName)) {
            return [pos, true];
        }

        const [child, childPos] = parseElement(str, startTagPos, tagName);
...
function parseElement(str, pos, tagName) {
    ...
    let isOmitClosing = false;
    if (!voidHtmlTags.includes(tagName.toLowerCase())) {
        const [newPos, closingOmit] = parseElements(str, pos, element);
        pos = newPos;
        isOmitClosing = closingOmit;
    }

    pos = eatWhiteSpace(str, pos);

    // Parse end tag of a non-void tag
    if (!isOmitClosing && !voidHtmlTags.includes(tagName.toLowerCase())) {
        const [endTagName, endTagPos] = parseEndTag(str, pos);
        if (endTagName === tagName) {
            pos = endTagPos;
        } else if (!closeIfLastChild.includes(tagName)) {
